...
# --- 洞穴配置 (v1.9.5 Imperial Total Blackout) ---
...
<style>
    ...
    /* 2. 暴力消除所有白框背景 (Target all Streamlit internal IDs) */
    ...
    /* 4. 输入文字高亮 */
    input[data-testid="stWidgetLabel-hidden"] {
...
<div style="display: flex; justify-content: center; align-items: center; height: 50px;">
    <button id="mic" style="background: transparent; border: none; cursor: pointer; padding: 0;">
        <svg id="mic-icon" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#444" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
...
<script>
    ...
    btn.onclick = () => {
        if (!('webkitSpeechRecognition' in window)) { alert("此神镜(浏览器)不支持语音感应"); return; }
        const recognition = new webkitSpeechRecognition();
        recognition.lang = 'zh-CN';
        recognition.onstart = () => { icon.style.stroke = '#00F2FF'; icon.style.filter = 'drop-shadow(0 0 15px #00F2FF)'; };
        ...
        recognition.onend = () => { icon.style.stroke = '#444'; icon.style.filter = 'none'; };
        recognition.start();
    };
</script>
"""
...
col_in, col_mic, col_go = st.columns([7, 1, 1])
with col_in:
        ...
        with st.spinner("正在捕捉天机..."):
            ...
            <div class="result-card">
                <div style="text-align: center; font-family: monospace; color: #1a1a1a; font-size: 0.8rem; margin-bottom: 3rem; letter-spacing: 3px;">
                    ESTABLISHED_TRACE: ({M}+{D}+{H}+{N}-3) MOD 6 = {res_idx}
                </div>
                ...
                <div style="color: #888; text-align: center; margin-top: 4rem; font-size: 1.3rem; line-height: 2.2;">
                    <span style="color: #333; font-size: 0.8rem; letter-spacing: 5px;">[ 所求之事 ]</span><br>
